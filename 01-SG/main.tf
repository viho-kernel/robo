module "mongodb" {
    ...
    source = "git::https://github.com/viho-kernel/terraform-aws-securitygroup.git?ref=main"
    project = var.project
...
resource "aws_security_group_rule" "frontend_vpn_ssh" {
  count = length(var.frontend_ssh_ports_vpn)
  type                     = "ingress"
  from_port                = var.frontend_ssh_ports_vpn[count.index]
  to_port                  = var.frontend_ssh_ports_vpn[count.index]
  protocol                 = "tcp"
  source_security_group_id = module.vpn.sg_id
...
resource "aws_security_group_rule" "frontend_bastion_ssh" {
  count = length(var.frontend_ssh_ports_bastion)
  type                     = "ingress"
  from_port                = var.frontend_ssh_ports_bastion[count.index]
  to_port                  = var.frontend_ssh_ports_bastion[count.index]
  protocol                 = "tcp"
  source_security_group_id = module.bastion.sg_id
  security_group_id        = module.frontend.sg_id
}
